...
export default function AdminDashboard() {
  ...
  const payments = mockPendingPayments;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Card 1 */}
        <div className="bg-card rounded-xl p-5 border border-border shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-1 mt-1 text-primary">
              ...
              <span className="text-xs font-medium">{stats.ordersTrend}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card rounded-xl p-5 border border-border shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-1 mt-1 text-primary">
              <TrendingUp className="size-3.5" />
              <span className="text-xs font-medium">{stats.salesTrend}</span>
            </div>
          </div>
        </div>

        {/* Card 3 - Pending Payments (highlighted) */}
        <div className="bg-card rounded-xl p-5 border border-destructive/30 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
      ...
      <div className="bg-destructive/5 rounded-xl border border-destructive/20 shadow-sm p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card rounded-xl border border-border shadow-sm p-5">
          ...
          <div className="flex items-center justify-center min-h-[280px] relative">
            {/* CSS Donut */}
            <div
              ...
              style={{
                background: `conic-gradient(
                  ...
                  ${statuses[1].color} ${statuses[0].percentage}% ${statuses[0].percentage + statuses[1].percentage}%,
                  ${statuses[2].color} ${statuses[0].percentage + statuses[1].percentage}% ${statuses[0].percentage + statuses[1].percentage + statuses[2].percentage}%,
                  ${statuses[3].color} ${statuses[0].percentage + statuses[1].percentage + statuses[2].percentage}% 100%
                )`,
              }}
            >
            ...
            {/* Legend */}
            <div className="hidden sm:flex flex-col gap-2 ml-8">
              {statuses.map((s) => (
        ...
        {/* Recent Activity */}
        <div className="bg-card rounded-xl border border-border shadow-sm p-5 flex flex-col max-h-[400px]">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-heading text-lg font-semibold text-foreground">
              Actividad Reciente
            </h2>
...
function PendingPaymentCard({ payment }: { payment: PendingPayment }) {
  return (
    <div className="bg-card rounded-lg p-4 border border-border shadow-sm flex flex-col justify-between">
      ...
      <div className="flex gap-2">
        <button className="flex-1 bg-primary text-primary-foreground py-2 rounded-lg text-sm font-medium hover:bg-accent transition-colors flex items-center justify-center gap-1.5 active:scale-[0.98]">
          <CheckCircle className="size-4" />
          Verificar
        </button>
        <button className="flex-1 bg-surface-container text-destructive py-2 rounded-lg border border-border text-sm font-medium hover:bg-destructive/10 transition-colors flex items-center justify-center gap-1.5 active:scale-[0.98]">
